...
- **`WRALM_Load_More`** — `wp_enqueue_scripts` (підключає `dist/js/...`, віддає `loadmore_params`: `resturl`, `ajaxurl` (legacy), `nonce` = `wp_rest`). Ендпоінт списку — **REST GET** `wralm/v1/list` (`handle_rest` → `build_list_response` → `WP_REST_Response` з `Cache-Control: public, max-age=30`, щоб reverse-proxy/CDN кешували). Legacy `wp_ajax_loadmore` / `_nopriv` лишається тонким shim над тим самим `build_list_response`. `build_list_response(array $params)` делегує в `WRALM_Query_Config::from_request`, рендерить картки й віддає `['html', 'pagination', 'max_page', 'canonical_url']` (`canonical_url` — точний URL стану фільтри+сторінка, `WRALM_Pagination::page_url()` з тих самих `base`/`format`/`add_args`, що й пагінація; `add_args` = `filter_query_args()`, порожньо коли `sync_filters_url=false`). **Без nonce-гейта** — анонімний nonce не секрет, а список опублікованих постів не CSRF-ціль; зловживання обмежує `rest_permission()` — per-IP rate limit (transient, default 60/хв, filter `wralm_rate_limit`, `429` при перевищенні). JS шле GET на `resturl` з `X-WP-Nonce` (тільки щоб залогінений глядач резолвився).
...
- **Namespaced URL-параметри фільтрів.** Таксономійні фільтри в URL — `filter_<taxonomy>=slug,slug` (не голе `<taxonomy>=`), плюс `filter_search`. Голе `?product_cat=a,b` збігається з query-var WooCommerce → WP робить 301 на `%2C`-версію й міняє контекст; namespaced `filter_product_cat` не query-var, тож ні 301, ні 404, ні стрибків формату пагінації. `get_queried_object()`-терм завжди справжній архів. `filter.php` не робить `exclude` queried-терму — усі кнопки термів рендеряться завжди. Серіалізація в `WRALM_Query_Config::filter_query_args()`; читання в JS `readUrlState()`. **`from_atts` теж читає ці GET-параметри** (`filter_<taxonomy>=`, `filter_search`) коли `sync_filters_url` — тож забукмарканий фільтрований URL рендериться відфільтрованим на сервері. `render_posts` кладе відрендерений стан у `data-init-filters` на `.ajax_row_holder`; JS `restoreFromUrl()` порівнює з адресним рядком і **пропускає** початковий re-fetch, якщо збіг (лишається тільки sync DOM панелі).
