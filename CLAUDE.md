...
**Кнопка «All» (`.wr-filters__item--all`).** Клік скидає лише категорійні фільтри (кнопки + селекти), не чіпає пошук/сортування, сабмітить форму. Активна автоматично коли знято останній фільтр (у т.ч. в single-select режимі `data-multiply="false"` — клік по активній кнопці знімає її й вертає «All»). Клік по «All» коли вона вже єдина активна — no-op (`e.preventDefault()`, без запиту). Окрема `.wr-filters__clear` чистить усе.
