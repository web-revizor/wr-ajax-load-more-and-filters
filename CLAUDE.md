...
- **`WRALM_Pagination::links()`** — статичний генератор розмітки пагінації (`list` / `both` / `none` / default-кнопка). Коли передано явні `base` / `format` / `total` / `current` — працює лише з `$args`, не чіпаючи WP-глобалі (потрібно для AJAX). Honorить `sync_pagination_url` (при `false` усі `href` = `#`).
  ...
  - `WRALM_Pagination::page_url($base, $format, $page, $add_args, $fragment)` → рядок URL сторінки N (підстановка `%_%`/`%#%`, `add_query_arg($add_args)`, `%2C`→`,`; сторінка 1 без сегмента). Єдине джерело — `links()` і `canonical_url` у `WRALM_Load_More` кличуть його, тому формат не розходиться.
- **`WRALM_Query_Config::from_atts` — GET-параметр ≠ архів.** Якщо `get_queried_object()` — це `WP_Term`, а в `$_GET` є ключ з іменем його таксономії (`?product_cat=…`), це наш фільтр, який WooCommerce/WP підняв у головний запит, а не архів: `archive_term_id/taxonomy` не виставляються, `archive_context=false`. Без цього кнопка того терму зникала з панелі (`filter.php` більше **не** робить `exclude` queried-терму) і емітився хибний pretty `/page/N/`.
