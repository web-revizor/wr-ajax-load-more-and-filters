...
- **`WRALM_Shortcode`** — `[all_posts_ajax]` (список + пагінація) і `[all_posts_ajax_filters]` (панель фільтрів/пошуку/сортування).
  ...
  - `[all_posts_ajax_filters]` будує `WRALM_Filter_Config` і `require`-ить в'юхи `inc/views/filter.php` / `inc/views/order.php`, які читають об'єкт `$config` **напряму** (жодної глобалі `$load_more_variables` — прибрано). На `.ajax_filters_wrapper` — лише `data-filter-id` (JS читає URL-sync-прапорці з `.ajax_row`, не з панелі).
