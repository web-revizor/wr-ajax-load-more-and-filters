...
- **`WRALM_Pagination::links()`** — статичний генератор розмітки пагінації (`list` / `both` / `none` / default-кнопка). Коли передано явні `base` / `format` / `total` / `current` — працює лише з `$args`, не чіпаючи WP-глобалі (потрібно для AJAX). Honorить `sync_pagination_url` (при `false` усі `href` = `#`).
  ...
  - `WRALM_Pagination::page_url($base, $format, $page, $add_args, $fragment)` → рядок URL сторінки N (підстановка `%_%`/`%#%`, `add_query_arg($add_args)`; сторінка 1 без сегмента). Коми в multi-term значеннях повертаються літерально (`add_query_arg` їх кодує, `page_url` розкодовує) — pretty `/page/N/` дає канонічний шлях, тож `redirect_canonical` не перебудовує query й не кодує їх назад. Єдине джерело — `links()` і `canonical_url` у `WRALM_Load_More` кличуть його, тому формат не розходиться.
- **`WRALM_Query_Config::from_atts` — фасетні URL.** (1) Якщо `get_queried_object()` — `WP_Term`, а в `$_GET` є ключ з іменем його таксономії (`?product_cat=…`), archive **term-scope** не виставляється (це фільтр користувача, не подвоювати в `tax_query`) — але `archive_context` не чіпається. (2) Якщо `archive_context` ще `false`, і сторінка не `is_singular()`/`is_front_page()`, і серед ключів `$_GET` є реальна таксономія → `archive_context=true`: `?product_cat=courses,figma` 404-ить головний запит (кома-значення не збігається з жодним термом) і без цього пагінація стрибала б між `?paged=N` і `/page/N/` (WP canonical-redirect). `filter.php` більше **не** робить `exclude` queried-терму — усі кнопки термів рендеряться завжди.
